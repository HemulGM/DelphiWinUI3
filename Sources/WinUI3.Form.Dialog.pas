...
uses
  ...
  WinUI3.Frame.Dialog.ColorPicker, WinUI3.Frame.Dialog.Font, WinUI3.Dialogs;
...
class function TFormDialogs.Execute(Owner: TCustomForm; Params: TDialogFontParams): TDialogFontResult;
begin
  var BodyFrame := TFrameDialogFont.Create(Owner);
  try
    BodyFrame.Fill(Params);
    var Data: TDialogFramedParams;
    Data.Title := Params.Title;
    ...
    Result.Result := FResult.Result;
    Result.IsChecked := FResult.IsChecked;
    Result.Family := BodyFrame.Family;
    Result.Size := BodyFrame.Size;
    Result.StyleExt := BodyFrame.StyleExt;
  finally
    BodyFrame.Free;
  end;
end;
